...
### btgenbot.py usage
Client application with GUI that, when provided with a complete one-shot example (comprising a description and its corresponding behavior tree), it generates the corresponding behavior tree for a new task. After generating the behavior tree, the application saves it to a file and initiates its transmission to the remote location of a robot for immediate execution.
